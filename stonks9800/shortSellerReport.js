...
// Based on academic research: Ljungqvist & Qin (2021), Wong & Zhao (2017), Putniņš (2024)
//
...
const SSR = (function() {
  ...
  function calculateReversalProbability(stock, verbose = true) {
    ...
    if (insiderBoost.isClusterBuy) {
      ...
      runningProb += boost;
      breakdown.push(`  + CLUSTER Insider Buying (${insiderBoost.buyCount} insiders): +${(boost * 100).toFixed(0)}% → ${(runningProb * 100).toFixed(0)}%`);
      ...
      breakdown.push(`  + Insider Buying ${details}: +${(boost * 100).toFixed(0)}% → ${(runningProb * 100).toFixed(0)}%`);
    } else {
    ...
    if (stock.auditorQuality === 'big4') {
      ...
      runningProb += boost;
      breakdown.push(`  + Auditor (${stock.auditorName} - Big 4): +${(boost * 100).toFixed(0)}% → ${(runningProb * 100).toFixed(0)}%`);
    } else {
    ...
    if (runningProb !== finalProb) {
      breakdown.push(`  CLAMPED: ${(runningProb * 100).toFixed(0)}% → ${(finalProb * 100).toFixed(0)}% (range: 5%-75%)`);
    }
    ...
    stockList.forEach(stock => {
      if (!stock.shortReportPhase && !stock.crashPhase && random() < 0.005) {
        ...
        const fraudHeadlines = [
          `🔴 ${seller.name}: "${stock.symbol} Fabricated 40% of Reported Revenue"`,
